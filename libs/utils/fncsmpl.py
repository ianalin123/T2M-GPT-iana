"""Somewhat opinionated wrapper for the SMPLH/SMPLX body model.

Very little of it is specific to SMPLH/SMPLX. This could very easily be adapted for other models in SMPL family.

We break down the SMPLH/SMPLX into four stages, each with a corresponding data structure:
...
class SmplModel(TensorDataclass):
    ...
    For SMPLH/SMPL, 1 root + 21 body joints + 2 * 15 hand joints."""
    ...
    def with_shape(self, betas: Float[Tensor, "*#batch n_betas"]) -> SmplShaped:
...
class SmplShaped(TensorDataclass):
    """The SMPLH/SMPLX body model with a body shape applied."""
    ...
    def with_pose_decomposed(
        ...
        T_world_root: Float[Tensor, "*#batch 7"] | None = None,
        left_hand_quats: Float[Tensor, "*#batch 15 4"] | None = None,
        right_hand_quats: Float[Tensor, "*#batch 15 4"] | None = None,
        jaw_quats: Float[Tensor, "*#batch 1 4"] | None = None,
        left_eye_quats: Float[Tensor, "*#batch 1 4"] | None = None,
        right_eye_quats: Float[Tensor, "*#batch 1 4"] | None = None,
        is_only_body: bool = False,
    ) -> SmplShapedAndPosed:
        """Pose our SMPLH/SMPLX body model. Returns a set of joint and vertex outputs."""

        if is_only_body:
            local_quats = body_quats
            num_joints = 21
        else:
            num_joints = self.body_model.get_num_joints()
            batch_axes = body_quats.shape[:-2]
            if T_world_root is None:
                T_world_root = body_quats.new_zeros((*batch_axes, 7))
                T_world_root[..., 0] = 1.0
            ...
            if jaw_quats is None:
                jaw_quats = body_quats.new_zeros((*batch_axes, 1, 4))
                jaw_quats[..., 0] = 1.0
            if left_eye_quats is None:
                left_eye_quats = body_quats.new_zeros((*batch_axes, 1, 4))
                left_eye_quats[..., 0] = 1.0
            if right_eye_quats is None:
                right_eye_quats = body_quats.new_zeros((*batch_axes, 1, 4))
                right_eye_quats[..., 0] = 1.0
            ...
            if num_joints == 51:
                ...
                # SMPLX
                local_quats = broadcasting_cat(
                    [body_quats, jaw_quats, left_eye_quats, right_eye_quats, 
                    left_hand_quats, right_hand_quats], dim=-2
                )
            else:
                raise ValueError(f"Unsupported number of joints: {num_joints}")

        assert local_quats.shape[-2:] == (num_joints, 4), f"local_quats.shape[-2:] == (num_joints, 4), but got {local_quats.shape[-2:]}"
        return self.with_pose(T_world_root, local_quats, is_only_body=is_only_body)
    ...
    def with_pose(
        ...
        Ts_world_joint = forward_kinematics(
            ...
            t_parent_joint=self.t_parent_joint[..., :num_joints, :],
            parent_indices=self.body_model.parent_indices[:num_joints],
        )
        assert Ts_world_joint.shape[-2:] == (num_joints, 7), f"Ts_world_joint.shape[-2:] == (num_joints, 7), but got {Ts_world_joint.shape[-2:]}"
...
class SmplMesh(TensorDataclass):
    """Outputs from the SMPLH/SMPLX model."""
