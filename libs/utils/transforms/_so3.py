...
class SO3(_base.SOBase):
    ...
    @staticmethod
    def slerp(t: Tensor, so3_0: SO3, so3_1: SO3) -> SO3:
        assert so3_0.wxyz.shape == so3_1.wxyz.shape
        dot = (so3_0.wxyz * so3_1.wxyz).sum(-1, keepdim=True)
        dot = torch.clamp(dot, -1.0, 1.0)

        # Ensure the shortest path
        neg = dot < 0
        so3_1_ = torch.where(neg, -so3_1.wxyz, so3_1.wxyz)
        ...
        s0 = torch.where(
            small_angle,
            1.0 - t,
            torch.sin((1 - t) * theta_0) / sin_theta_0
        )
        s1 = torch.where(
            small_angle,
            t,
            torch.sin(t * theta_0) / sin_theta_0
        )
        return SO3(wxyz=s0 * so3_0.wxyz + s1 * so3_1_)
    ...
    def as_6d(self) -> Tensor:
        """Convert SO3 rotation to continuous 6D representation.
        
        The 6D representation is obtained by taking the first two columns of the
        rotation matrix. This provides a continuous representation of 3D rotations.
        
        Returns:
            Tensor: 6D representation of shape (..., 6)
        """
        matrix = self.as_matrix()
        return torch.cat([matrix[..., :, 0], matrix[..., :, 1]], dim=-1)

    @classmethod
    def from_6d(cls, x: Tensor) -> SO3:
        """Convert 6D representation back to SO3 rotation.
        
        Args:
            x: 6D representation of shape (..., 6)
        ...
        # Compute third column as cross product to ensure right-handed system
        # b3 = torch.cross(b1, b2, dim=-1)
        b3 = torch.nn.functional.normalize(torch.cross(b1, b2, dim=-1), dim=-1, eps=1e-6)
    ...
    def from_6d_to_matrix(x: Tensor) -> Tensor:
        # Get the first two columns
        a1, a2 = x[..., :3], x[..., 3:]
    ...
    def as_matrix(self) -> Tensor:
        ...
        norm_sq = torch.clamp(norm_sq, min=1e-7)
#if norm_sq.min() ==0 else norm_sq
