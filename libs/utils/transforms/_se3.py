...
class SE3(_base.SEBase[SO3]):
    ...
    def as_9d(self) -> Tensor:
        """Convert SE3 transform to continuous 9D representation (6D rotation + 3D translation).
        
        Returns:
            Tensor: 9D representation of shape (..., 9) where first 6 dimensions are
                  the continuous 6D rotation representation and last 3 are translation.
        """
        rot_6d = self.rotation().as_6d()
        translation = self.translation()
        return torch.cat([rot_6d, translation], dim=-1)

    @classmethod
    def from_9d(cls, x: Tensor) -> SE3:
        """Convert 9D representation (6D rotation + 3D translation) back to SE3 transform.
        
        Args:
            x: 9D representation of shape (..., 9) where first 6 dimensions are
               the continuous 6D rotation representation and last 3 are translation.
            
        Returns:
            SE3: Transform in SE3
        """
        rot_6d = x[..., :6]
        translation = x[..., 6:]
        
        rotation = SO3.from_6d(rot_6d)
        return cls.from_rotation_and_translation(rotation, translation)
